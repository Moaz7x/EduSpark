...
/// Use case for deleting a PomodoroSession entity.
///
/// This use case handles the business logic for removing a PomodoroSession
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeletePomodoroSessionUseCase extends BaseUseCase<bool, IdParams> {
  final PomodoroSessionRepository repository;

  DeletePomodoroSessionUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('PomodoroSession with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete PomodoroSession: ${e.toString()}', e));
    }
  }
