...
/// Use case for deleting a BreakSession entity.
///
/// This use case handles the business logic for removing a BreakSession
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteBreakSessionUseCase extends BaseUseCase<bool, IdParams> {
  final BreakSessionRepository repository;

  DeleteBreakSessionUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('BreakSession with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete BreakSession: ${e.toString()}', e));
    }
  }
