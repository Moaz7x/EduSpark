...
/// Repository implementation for Category feature.
///
...
class CategoryRepositoryImpl implements CategoryRepository {
  ...
  Future<Either<Failure, CategoryEntity>> create(CategoryEntity entity) async {
    try {
      ...
      final result = await localDataSource.create<CategoryModel>(model);
      return result.fold(
  ...
  Future<Either<Failure, CategoryEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<CategoryModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  Future<Either<Failure, List<CategoryEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<CategoryModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  Future<Either<Failure, CategoryEntity>> update(CategoryEntity entity) async {
    try {
      final model = CategoryModel.fromEntity(entity);
      final result = await localDataSource.update<CategoryModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<CategoryEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<CategoryModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  Future<Either<Failure, List<CategoryEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<CategoryModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  Future<Either<Failure, List<CategoryEntity>>> getPaginated(
    ...
    try {
      final result = await localDataSource.getPaginated<CategoryModel>(
  ...
  @override
  Future<Either<Failure, List<CategoryEntity>>> getFiltered(
  ...
  Future<List<TaskEntity>> getTasks(int categoryId) async {
    try {
      final filters = {'categoryId': categoryId};
      final result = await localDataSource.getFiltered<TaskModel>(filters, 'task');
      return result.fold(
        (failure) => throw RepositoryException(failure.message, null),
        (models) => models.map((TaskModel model) => model.toEntity()).toList(),
      );
    } catch (e) {
      throw RepositoryException('Failed to get tasks for Category', e);
    }
  }
  ...
  Future<Either<Failure, CategoryEntity>> loadWithRelationships(
  ...
  Future<Either<Failure, bool>> saveWithRelationships(CategoryEntity entity,
