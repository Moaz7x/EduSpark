...
class StatisticsRepositoryImpl implements StatisticsRepository {
  ...
  static const String _entityType = 'statistics';

  @override
  Future<Either<Failure, StatisticsEntity>> create(StatisticsEntity entity) async {
    try {
      final model = StatisticsModel.fromEntity(entity);
      final result = await localDataSource.create<StatisticsModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, StatisticsEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<StatisticsModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<StatisticsEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<StatisticsModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, StatisticsEntity>> update(StatisticsEntity entity) async {
    try {
      final model = StatisticsModel.fromEntity(entity);
      final result = await localDataSource.update<StatisticsModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<StatisticsEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<StatisticsModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<StatisticsEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<StatisticsModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<StatisticsEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<StatisticsModel>(
  ...
  @override
  Future<Either<Failure, List<StatisticsEntity>>> getFiltered(
  ...
  Future<Either<Failure, StatisticsEntity>> loadWithRelationships(
      StatisticsEntity entity) async {
    try {
      final model = StatisticsModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(StatisticsEntity entity,
