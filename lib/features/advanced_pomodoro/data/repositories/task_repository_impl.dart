...
/// Repository implementation for Task feature.
///
...
class TaskRepositoryImpl implements TaskRepository {
  ...
  static const String _entityType = 'task';

  @override
  Future<Either<Failure, TaskEntity>> create(TaskEntity entity) async {
    try {
      final model = TaskModel.fromEntity(entity);
      final result = await localDataSource.create<TaskModel>(model);
      return result.fold(
  ...
  Future<Either<Failure, TaskEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<TaskModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  Future<Either<Failure, List<TaskEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<TaskModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, TaskEntity>> update(TaskEntity entity) async {
    try {
      final model = TaskModel.fromEntity(entity);
      final result = await localDataSource.update<TaskModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<TaskEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<TaskModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  Future<Either<Failure, List<TaskEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<TaskModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<TaskEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<TaskModel>(
  ...
  @override
  Future<Either<Failure, List<TaskEntity>>> getFiltered(
  ...
  /// Gets all pomodorosessions for a specific task
  Future<List<PomodoroSessionEntity>> getPomodorosessions(int taskId) async {
    try {
      final filters = {'taskId': taskId};
      final result = await localDataSource.getFiltered<PomodoroSessionModel>(filters, 'pomodoro_session');
      return result.fold(
        (failure) => throw RepositoryException(failure.message, null),
        (models) => models.map((PomodoroSessionModel model) => model.toEntity()).toList(),
      );
    } catch (e) {
      throw RepositoryException('Failed to get pomodorosessions for Task', e);
    }
  }
  ...
  Future<Either<Failure, TaskEntity>> loadWithRelationships(
      TaskEntity entity) async {
    try {
      final model = TaskModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(TaskEntity entity,
