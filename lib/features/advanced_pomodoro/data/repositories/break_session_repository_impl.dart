...
import '../../domain/repositories/break_session_repository.dart';
...
/// Repository implementation for BreakSession feature.
///
...
class BreakSessionRepositoryImpl implements BreakSessionRepository {
  final LocalDataSource localDataSource;

  BreakSessionRepositoryImpl(this.localDataSource);

  static const String _entityType = 'break_session';

  @override
  Future<Either<Failure, BreakSessionEntity>> create(BreakSessionEntity entity) async {
    try {
      final model = BreakSessionModel.fromEntity(entity);
      final result = await localDataSource.create<BreakSessionModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, BreakSessionEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<BreakSessionModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<BreakSessionEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<BreakSessionModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, BreakSessionEntity>> update(BreakSessionEntity entity) async {
    try {
      final model = BreakSessionModel.fromEntity(entity);
      final result = await localDataSource.update<BreakSessionModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<BreakSessionEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<BreakSessionModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<BreakSessionEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<BreakSessionModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<BreakSessionEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<BreakSessionModel>(
  ...
  @override
  Future<Either<Failure, List<BreakSessionEntity>>> getFiltered(
  ...
  Future<Either<Failure, BreakSessionEntity>> loadWithRelationships(
      BreakSessionEntity entity) async {
    try {
      final model = BreakSessionModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(BreakSessionEntity entity,
    ...
    try {
      ...
      if (childEntities != null) {
        childModels = childEntities.map((child) {
          ...
          if (child is PomodoroSessionEntity) {
            return PomodoroSessionModel.fromEntity(child);
          }
