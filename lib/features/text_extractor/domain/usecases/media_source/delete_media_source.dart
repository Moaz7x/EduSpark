...
/// Use case for deleting a MediaSource entity.
///
/// This use case handles the business logic for removing a MediaSource
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteMediaSourceUseCase extends BaseUseCase<bool, IdParams> {
  final MediaSourceRepository repository;

  DeleteMediaSourceUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('MediaSource with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete MediaSource: ${e.toString()}', e));
    }
  }
