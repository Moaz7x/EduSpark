...
/// Use case for deleting a UserSettings entity.
///
/// This use case handles the business logic for removing a UserSettings
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteUserSettingsUseCase extends BaseUseCase<bool, IdParams> {
  final UserSettingsRepository repository;

  DeleteUserSettingsUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('UserSettings with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete UserSettings: ${e.toString()}', e));
    }
  }
