...
/// Use case for deleting a ExtractedText entity.
///
/// This use case handles the business logic for removing a ExtractedText
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteExtractedTextUseCase extends BaseUseCase<bool, IdParams> {
  final ExtractedTextRepository repository;

  DeleteExtractedTextUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('ExtractedText with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete ExtractedText: ${e.toString()}', e));
    }
  }
