...
/// Repository implementation for UserSettings feature.
///
...
class UserSettingsRepositoryImpl implements UserSettingsRepository {
  ...
  UserSettingsRepositoryImpl(this.localDataSource);

  static const String _entityType = 'user_settings';

  @override
  Future<Either<Failure, UserSettingsEntity>> create(UserSettingsEntity entity) async {
    try {
      final model = UserSettingsModel.fromEntity(entity);
      final result = await localDataSource.create<UserSettingsModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, UserSettingsEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<UserSettingsModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<UserSettingsEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<UserSettingsModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, UserSettingsEntity>> update(UserSettingsEntity entity) async {
    try {
      final model = UserSettingsModel.fromEntity(entity);
      final result = await localDataSource.update<UserSettingsModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<UserSettingsEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<UserSettingsModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<UserSettingsEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<UserSettingsModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<UserSettingsEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<UserSettingsModel>(
  ...
  @override
  Future<Either<Failure, List<UserSettingsEntity>>> getFiltered(
  ...
  Future<Either<Failure, UserSettingsEntity>> loadWithRelationships(
      UserSettingsEntity entity) async {
    try {
      final model = UserSettingsModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(UserSettingsEntity entity,
