...
import '../../domain/repositories/media_source_repository.dart';
...
/// Repository implementation for MediaSource feature.
///
...
class MediaSourceRepositoryImpl implements MediaSourceRepository {
  ...
  MediaSourceRepositoryImpl(this.localDataSource);

  static const String _entityType = 'media_source';

  @override
  Future<Either<Failure, MediaSourceEntity>> create(MediaSourceEntity entity) async {
    try {
      final model = MediaSourceModel.fromEntity(entity);
      final result = await localDataSource.create<MediaSourceModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, MediaSourceEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<MediaSourceModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<MediaSourceEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<MediaSourceModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, MediaSourceEntity>> update(MediaSourceEntity entity) async {
    try {
      final model = MediaSourceModel.fromEntity(entity);
      final result = await localDataSource.update<MediaSourceModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<MediaSourceEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<MediaSourceModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<MediaSourceEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<MediaSourceModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<MediaSourceEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<MediaSourceModel>(
  ...
  @override
  Future<Either<Failure, List<MediaSourceEntity>>> getFiltered(
  ...
  /// Gets all textextractionjobs for a specific mediasource
  Future<List<TextExtractionJobEntity>> getTextextractionjobs(int mediasourceId) async {
    try {
      final filters = {'mediasourceId': mediasourceId};
      final result = await localDataSource.getFiltered<TextExtractionJobModel>(filters, 'text_extraction_job');
      return result.fold(
        (failure) => throw RepositoryException(failure.message, null),
        (models) => models.map((TextExtractionJobModel model) => model.toEntity()).toList(),
      );
    } catch (e) {
      throw RepositoryException('Failed to get textextractionjobs for MediaSource', e);
    }
  }
  ...
  Future<Either<Failure, MediaSourceEntity>> loadWithRelationships(
      MediaSourceEntity entity) async {
    try {
      final model = MediaSourceModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(MediaSourceEntity entity,
