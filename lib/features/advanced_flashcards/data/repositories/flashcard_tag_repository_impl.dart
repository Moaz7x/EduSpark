...
import '../../domain/repositories/flashcard_tag_repository.dart';
...
/// Repository implementation for FlashcardTag feature.
///
...
class FlashcardTagRepositoryImpl implements FlashcardTagRepository {
  final LocalDataSource localDataSource;

  FlashcardTagRepositoryImpl(this.localDataSource);

  static const String _entityType = 'flashcard_tag';

  @override
  Future<Either<Failure, FlashcardTagEntity>> create(FlashcardTagEntity entity) async {
    try {
      final model = FlashcardTagModel.fromEntity(entity);
      final result = await localDataSource.create<FlashcardTagModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, FlashcardTagEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<FlashcardTagModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<FlashcardTagEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<FlashcardTagModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, FlashcardTagEntity>> update(FlashcardTagEntity entity) async {
    try {
      final model = FlashcardTagModel.fromEntity(entity);
      final result = await localDataSource.update<FlashcardTagModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<FlashcardTagEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<FlashcardTagModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<FlashcardTagEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<FlashcardTagModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<FlashcardTagEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<FlashcardTagModel>(
  ...
  @override
  Future<Either<Failure, List<FlashcardTagEntity>>> getFiltered(
  ...
  Future<Either<Failure, FlashcardTagEntity>> loadWithRelationships(
      FlashcardTagEntity entity) async {
    try {
      final model = FlashcardTagModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(FlashcardTagEntity entity,
