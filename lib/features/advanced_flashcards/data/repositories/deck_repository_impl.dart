...
/// Repository implementation for Deck feature.
///
...
class DeckRepositoryImpl implements DeckRepository {
  ...
  static const String _entityType = 'deck';

  @override
  Future<Either<Failure, DeckEntity>> create(DeckEntity entity) async {
    try {
      final model = DeckModel.fromEntity(entity);
      final result = await localDataSource.create<DeckModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, DeckEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<DeckModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<DeckEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<DeckModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, DeckEntity>> update(DeckEntity entity) async {
    try {
      final model = DeckModel.fromEntity(entity);
      final result = await localDataSource.update<DeckModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<DeckEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<DeckModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<DeckEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<DeckModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<DeckEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<DeckModel>(
  ...
  @override
  Future<Either<Failure, List<DeckEntity>>> getFiltered(
  ...
  Future<List<FlashcardEntity>> getFlashcards(int deckId) async {
    try {
      final filters = {'deckId': deckId};
      final result = await localDataSource.getFiltered<FlashcardModel>(filters, 'flashcard');
      return result.fold(
        (failure) => throw RepositoryException(failure.message, null),
        (models) => models.map((FlashcardModel model) => model.toEntity()).toList(),
      );
    } catch (e) {
      throw RepositoryException('Failed to get flashcards for Deck', e);
    }
  }
  ...
  Future<Either<Failure, DeckEntity>> loadWithRelationships(
      DeckEntity entity) async {
    try {
      final model = DeckModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(DeckEntity entity,
