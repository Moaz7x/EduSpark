...
/// Repository implementation for Tag feature.
///
...
class TagRepositoryImpl implements TagRepository {
  ...
  @override
  Future<Either<Failure, TagEntity>> create(TagEntity entity) async {
    try {
      final model = TagModel.fromEntity(entity);
      final result = await localDataSource.create<TagModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, TagEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<TagModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<TagEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<TagModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, TagEntity>> update(TagEntity entity) async {
    try {
      final model = TagModel.fromEntity(entity);
      final result = await localDataSource.update<TagModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<TagEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<TagModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<TagEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<TagModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<TagEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<TagModel>(
  ...
  @override
  Future<Either<Failure, List<TagEntity>>> getFiltered(
  ...
  /// Gets all flashcardtags for a specific tag
  Future<List<FlashcardTagEntity>> getFlashcardtags(int tagId) async {
    try {
      final filters = {'tagId': tagId};
      ...
      throw RepositoryException('Failed to get flashcardtags for Tag', e);
    }
  }
  ...
  Future<Either<Failure, TagEntity>> loadWithRelationships(
      TagEntity entity) async {
    try {
      final model = TagModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(TagEntity entity,
