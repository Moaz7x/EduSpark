...
/// Use case for deleting a Flashcard entity.
///
/// This use case handles the business logic for removing a Flashcard
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteFlashcardUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Flashcard with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Flashcard: ${e.toString()}', e));
    }
  }
