...
/// Use case for deleting a FlashcardTag entity.
///
/// This use case handles the business logic for removing a FlashcardTag
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteFlashcardTagUseCase extends BaseUseCase<bool, IdParams> {
  final FlashcardTagRepository repository;

  DeleteFlashcardTagUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('FlashcardTag with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete FlashcardTag: ${e.toString()}', e));
    }
  }
