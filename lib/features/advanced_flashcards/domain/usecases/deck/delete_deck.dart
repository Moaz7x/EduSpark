...
/// Use case for deleting a Deck entity.
///
/// This use case handles the business logic for removing a Deck
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteDeckUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Deck with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Deck: ${e.toString()}', e));
    }
  }
