...
/// Use case for deleting a Tag entity.
///
/// This use case handles the business logic for removing a Tag
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteTagUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Tag with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Tag: ${e.toString()}', e));
    }
  }
