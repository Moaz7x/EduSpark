...
/// Use case for deleting a Media entity.
///
/// This use case handles the business logic for removing a Media
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteMediaUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Media with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Media: ${e.toString()}', e));
    }
  }
