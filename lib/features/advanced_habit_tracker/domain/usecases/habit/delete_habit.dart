...
/// Use case for deleting a Habit entity.
///
/// This use case handles the business logic for removing a Habit
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteHabitUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Habit with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Habit: ${e.toString()}', e));
    }
  }
