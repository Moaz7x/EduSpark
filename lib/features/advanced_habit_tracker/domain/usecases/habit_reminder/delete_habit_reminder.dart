...
/// Use case for deleting a HabitReminder entity.
///
/// This use case handles the business logic for removing a HabitReminder
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteHabitReminderUseCase extends BaseUseCase<bool, IdParams> {
  final HabitReminderRepository repository;

  DeleteHabitReminderUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('HabitReminder with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete HabitReminder: ${e.toString()}', e));
    }
  }
