...
/// Use case for deleting a HabitLog entity.
///
/// This use case handles the business logic for removing a HabitLog
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteHabitLogUseCase extends BaseUseCase<bool, IdParams> {
  final HabitLogRepository repository;

  DeleteHabitLogUseCase(this.repository);
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('HabitLog with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete HabitLog: ${e.toString()}', e));
    }
  }
