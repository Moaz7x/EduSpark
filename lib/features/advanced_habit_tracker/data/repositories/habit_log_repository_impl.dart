...
/// Repository implementation for HabitLog feature.
///
...
class HabitLogRepositoryImpl implements HabitLogRepository {
  ...
  HabitLogRepositoryImpl(this.localDataSource);

  static const String _entityType = 'habit_log';

  @override
  Future<Either<Failure, HabitLogEntity>> create(HabitLogEntity entity) async {
    try {
      final model = HabitLogModel.fromEntity(entity);
      final result = await localDataSource.create<HabitLogModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, HabitLogEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<HabitLogModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<HabitLogEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<HabitLogModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, HabitLogEntity>> update(HabitLogEntity entity) async {
    try {
      final model = HabitLogModel.fromEntity(entity);
      final result = await localDataSource.update<HabitLogModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<HabitLogEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<HabitLogModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<HabitLogEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<HabitLogModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<HabitLogEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<HabitLogModel>(
  ...
  @override
  Future<Either<Failure, List<HabitLogEntity>>> getFiltered(
  ...
  Future<Either<Failure, HabitLogEntity>> loadWithRelationships(
      HabitLogEntity entity) async {
    try {
      final model = HabitLogModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(HabitLogEntity entity,
