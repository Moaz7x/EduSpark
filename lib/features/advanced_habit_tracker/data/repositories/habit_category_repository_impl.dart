...
/// Repository implementation for HabitCategory feature.
///
...
class HabitCategoryRepositoryImpl implements HabitCategoryRepository {
  ...
  HabitCategoryRepositoryImpl(this.localDataSource);

  static const String _entityType = 'habit_category';

  @override
  Future<Either<Failure, HabitCategoryEntity>> create(HabitCategoryEntity entity) async {
    try {
      final model = HabitCategoryModel.fromEntity(entity);
      final result = await localDataSource.create<HabitCategoryModel>(model);
      return result.fold(
  ...
  @override
  Future<Either<Failure, HabitCategoryEntity?>> getById(int id) async {
    try {
      final result = await localDataSource.getById<HabitCategoryModel>(id, _entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<HabitCategoryEntity>>> getAll() async {
    try {
      final result = await localDataSource.getAll<HabitCategoryModel>(_entityType);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, HabitCategoryEntity>> update(HabitCategoryEntity entity) async {
    try {
      final model = HabitCategoryModel.fromEntity(entity);
      final result = await localDataSource.update<HabitCategoryModel>(model);
      return result.fold(
        (failure) => Left(failure),
  ...
  @override
  Future<Either<Failure, List<HabitCategoryEntity>>> getByField(
      String fieldName, dynamic value) async {
    try {
      final filters = {fieldName: value};
      final result =
          await localDataSource.getFiltered<HabitCategoryModel>(filters, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<HabitCategoryEntity>>> search(String query) async {
    try {
      final result =
          await localDataSource.search<HabitCategoryModel>(query, _entityType);
      return result.fold(
        (failure) => Left(failure),
        (models) => Right(models.map((model) => model.toEntity()).toList()),
      );
  ...
  @override
  Future<Either<Failure, List<HabitCategoryEntity>>> getPaginated(
      int page, int limit) async {
    try {
      final result = await localDataSource.getPaginated<HabitCategoryModel>(
  ...
  @override
  Future<Either<Failure, List<HabitCategoryEntity>>> getFiltered(
  ...
  Future<Either<Failure, HabitCategoryEntity>> loadWithRelationships(
      HabitCategoryEntity entity) async {
    try {
      final model = HabitCategoryModel.fromEntity(entity);
      final result =
  ...
  Future<Either<Failure, bool>> saveWithRelationships(HabitCategoryEntity entity,
    ...
    try {
      ...
      if (childEntities != null) {
        childModels = childEntities.map((child) {
          ...
          if (child is HabitEntity) {
            return HabitModel.fromEntity(child);
          }
