...
/// Use case for deleting a Todo entity.
///
/// This use case handles the business logic for removing a Todo
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteTodoUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Todo with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Todo: ${e.toString()}', e));
    }
  }
