...
/// Use case for deleting a Subtask entity.
///
/// This use case handles the business logic for removing a Subtask
/// entity from the system.
/// Returns Either&lt;Failure, bool&gt; for functional error handling.
class DeleteSubtaskUseCase extends BaseUseCase<bool, IdParams> {
    ...
    try {
      ...
      return existsResult.fold(
        ...
        (exists) {
          if (!exists) {
            return Left(
                ValidationFailure('Subtask with ID ${params.id} does not exist'));
          }
      ...
      return Left(UnknownFailure('Failed to delete Subtask: ${e.toString()}', e));
    }
  }
